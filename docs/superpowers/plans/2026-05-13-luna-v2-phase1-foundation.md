# Luna v2 Phase 1 — Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `Luna-terminal` with `alacritty_terminal` and `cosmic-text/swash` with `fontdue`, producing a terminal that renders glyphs correctly with no atlas/pixel artifacts.
...
|--------|------|----------------|
| Delete | `crates/Luna-terminal/` | already removed in cleanup |
| Modify | `Cargo.toml` | remove Luna-terminal member + cosmic-text/vte workspace deps; add fontdue + alacritty_terminal |
| Modify | `crates/Luna-renderer/Cargo.toml` | swap cosmic-text → fontdue |
| Modify | `crates/Luna-renderer/src/text.rs` | fontdue-based rasterizer + cell metrics |
| Modify | `crates/Luna-renderer/src/atlas.rs` | swap `CacheKey` → `GlyphKey`; add `gray_to_rgba` helper |
| Modify | `crates/Luna-renderer/src/renderer.rs` | use `GlyphBitmap`/`GlyphKey`; remove `swash_to_rgba` |
| Modify | `crates/Luna-ui/Cargo.toml` | remove luna_terminal; add alacritty_terminal + portable-pty |
| Modify | `crates/Luna-ui/src/pane.rs` | new Pane: holds `Term<EventProxy>` + PTY writer + dirty flag |
| Modify | `crates/Luna-app/Cargo.toml` | remove Luna-terminal; keep portable-pty (already transitive) |
| Modify | `crates/Luna-app/src/pane_ops.rs` | rewrite `create_pane()` for alacritty_terminal |
| Modify | `crates/Luna-app/src/render.rs` | read grid via `term.grid().display_iter()` |
| Modify | `crates/Luna-app/src/app.rs` | update PTY write, resize, focus event mode checks |
...
**Files:**
- Modify: `Cargo.toml`
- Modify: `crates/Luna-renderer/Cargo.toml`
- Modify: `crates/Luna-ui/Cargo.toml`
- Modify: `crates/Luna-app/Cargo.toml`
...
- [ ] **Step 2: Update Luna-renderer/Cargo.toml**
...
- [ ] **Step 3: Update Luna-ui/Cargo.toml**
...
Luna-renderer = { path = "../Luna-renderer" }
winit = { workspace = true }
```

- [ ] **Step 4: Update Luna-app/Cargo.toml**
...
authors.workspace = true
repository.workspace = true
description = "Luna terminal emulator — binary entry point"
...
[dependencies]
Luna-renderer = { path = "../Luna-renderer" }
Luna-ui = { path = "../Luna-ui" }
Luna-config = { path = "../Luna-config" }
alacritty_terminal = { workspace = true }
winit = { workspace = true }
wgpu = { workspace = true }
tracing = { workspace = true }
...
```bash
git add Cargo.toml crates/Luna-renderer/Cargo.toml crates/Luna-ui/Cargo.toml crates/Luna-app/Cargo.toml Cargo.lock
git commit -m "chore(deps): swap cosmic-text→fontdue, add alacritty_terminal, remove Luna-terminal"
```
...
**Files:**
- Modify: `crates/Luna-renderer/src/text.rs`
- Test: `crates/Luna-renderer/src/text.rs` (inline `#[cfg(test)]`)

- [ ] **Step 1: Write failing tests first**

Replace the entire `crates/Luna-renderer/src/text.rs`:
...
```bash
cargo test -p Luna-renderer --lib 2>&1 | head -30
```
...
```bash
git add crates/Luna-renderer/src/text.rs
...
**Files:**
- Modify: `crates/Luna-renderer/src/atlas.rs`
...
```bash
git add crates/Luna-renderer/src/atlas.rs
...
**Files:**
- Modify: `crates/Luna-renderer/src/renderer.rs`
...
```bash
cargo build -p Luna-renderer 2>&1 | head -40
```
...
```bash
cargo test -p Luna-renderer 2>&1
```
...
```bash
git add crates/Luna-renderer/src/renderer.rs
...
**Files:**
- Modify: `crates/Luna-ui/src/pane.rs`
...
- [ ] **Step 2: Try to compile luna-ui**

```bash
cargo build -p Luna-ui 2>&1 | head -40
```

The compile will fail because other luna-ui files still import luna_terminal types. Fix imports in each file:

- `crates/Luna-ui/src/lib.rs`: remove `luna_terminal` from any re-exports
- Any other file in Luna-ui that imports from `luna_terminal`: update or remove the import

- [ ] **Step 3: Commit**

```bash
git add crates/Luna-ui/src/pane.rs crates/Luna-ui/src/lib.rs
git commit -m "feat(ui): replace Luna-terminal Pane with alacritty_terminal Term"
```
...
pub fn find_pane<'a>(panes: &'a mut Vec<Pane>, id: luna_ui::pane::PaneId) -> Option<&'a mut Pane> {
...
```bash
cargo build -p Luna-app 2>&1 | head -40
```
...
```bash
git add crates/Luna-app/src/pane_ops.rs
...
```rust
{
    ...
    let new_size = luna_app::TermSize { cols: new_cols, rows: new_rows };
...
```bash
cargo build -p Luna-app 2>&1 | head -60
```
...
- Any remaining `luna_terminal::` imports → remove

- [ ] **Step 8: Commit**

```bash
git add crates/Luna-app/src/render.rs crates/Luna-app/src/app.rs
...
- [ ] **Step 2: Fix remaining luna_terminal references**

```bash
grep -r "luna_terminal\|Luna-terminal\|luna_terminal" crates/ --include="*.rs" -l
```
...
## Task 10: Smoke test — run Luna

- [ ] **Step 1: Build release**

```bash
cargo build -p Luna-app 2>&1 | tail -5
```
...
```bash
git add -A
git commit -m "feat: Luna v2 Phase 1 complete — alacritty_terminal + fontdue foundation"
```
...
**Files:**
- Modify: `crates/Luna-ui/src/pane.rs` (add inline test)

- [ ] **Step 1: Add pane unit tests**

Add to the end of `crates/Luna-ui/src/pane.rs`:
...
```bash
git add crates/Luna-ui/src/pane.rs
...
At this point Luna v2 Phase 1 is done:

- [x] `Luna-terminal` crate removed
...
**Next:** Phase 2 plan covers UI rework (tabs, splits cleanup, config). Phase 3 covers `luna-suggest` autosuggestions.
