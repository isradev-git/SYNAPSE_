# Luna v2 — Rewrite Design Spec

**Date:** 2026-05-13  
**Status:** Approved  
**Goal:** Rewrite Luna on a proven foundation to fix glyph rendering and deliver a fast, complete, local-first terminal emulator.

---

## 1. Problem Statement

Luna v1 has two blocking issues:
...
2. **Dev velocity too low** — maintaining custom VT parser, grid, PTY handling in Luna-terminal is expensive; bugs are hard to isolate.

**Solution:** Replace Luna-terminal with `alacritty_terminal` (battle-tested, Apache 2.0) and replace cosmic-text with `fontdue` (pure Rust, simple, reliable). Keep wgpu GPU pipeline and existing UI concepts.
...
```
luna-app (binary)
  ├── alacritty_terminal   ← external crate, replaces Luna-terminal
  ├── luna-renderer        ← wgpu 22 + fontdue (new renderer)
  ├── luna-ui              ← pane tree, tab bar, layout (rework)
  ├── luna-suggest         ← NEW: local history autosuggestions
  └── luna-config          ← TOML config, themes, keybinds (carry over)
```

**Luna-terminal crate is eliminated.**

### Dependency Rationale

| Component | v1 | v2 | Reason |
|-----------|----|----|--------|
| Terminal logic | Luna-terminal (custom) | `alacritty_terminal` | Years of VT compat, stable grid/scrollback |
...
| Autosuggestions | none | `luna-suggest` | New feature |
...
winit EventLoop (Poll mode)
  ...
  └── RedrawRequested
        ...
        │     ├── ghost text overlay (luna-suggest)
...
## 4. luna-suggest: Autosuggestions
...
Tab bar: top of window, same luna dark theme.
...
4 built-in (luna, dracula, catppuccin-mocha, tokyo-night). Custom themes via TOML. Hot-reload: `Ctrl+,`.
...
| Error | Behavior |
|-------|----------|
| No GPU adapter | `panic!("No GPU adapter found. Luna requires Vulkan/Metal/DX12.")` |
...
|-------|-------|-----------|
| **1 — Foundation** | Swap Luna-terminal → alacritty_terminal. Replace cosmic-text → fontdue. Renderer renders ASCII correctly. | Text displays without glyph artifacts |
...
| **3 — Autosuggestions** | luna-suggest crate, trie, ghost text overlay, Tab/→ accept | Ghost text shows from real shell history |
