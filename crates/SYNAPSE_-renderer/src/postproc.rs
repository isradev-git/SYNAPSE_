...
impl PostProcRenderer {
    pub fn new(device: Arc<Device>, format: wgpu::TextureFormat, width: u32, height: u32) -> Self {
    ...
    pub fn resize(&mut self, width: u32, height: u32, queue: &Queue) {
        self.offscreen_tex = Self::make_tex(
            &self.device,
            self.format,
            width,
            height,
        ...
        self.bloom_h_tex =
            Self::make_tex(&self.device, self.format, width, height, "SYNAPSE_ BloomH");
    ...
    pub fn render(
        ...
        if bloom_on {
            ...
            queue.write_buffer(
                &self.bloom_h_uniform_buf,
                0,
                bytemuck::bytes_of(&bloom_uniform),
            );
